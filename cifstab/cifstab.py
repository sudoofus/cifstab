...
class Cifstab():
    ...
    def systemdfile(self,args):
        template = { 'path':sys.argv[0], 'comment':'# Generated by cifstab' }
        notpresent = []
...
def main():
    defaultRetries = 3
    defaultWaitSecs = 5
    parser = argparse.ArgumentParser(description='cifstab {} | Command line utility for mounting cifs shares using encrypted passwords'.format(__version__))
    ...
    cifstab = Cifstab(retries=getattr(args,'retries',defaultRetries),waitsecs=getattr(args,'waitsecs',defaultWaitSecs))
    getattr(cifstab, args.subcommand)(args)
    cifstab.checkstatus()
    parser.exit(status=cifstab.status['error'])
